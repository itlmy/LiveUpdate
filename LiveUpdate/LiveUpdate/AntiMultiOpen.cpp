...
{
	HANDLE hMutex = OpenMutex(MUTEX_ALL_ACCESS, false, _T("mutex_liveupdate_46dad5a1cd758470"));
	if (!hMutex)
	{
		HANDLE hObject = ::CreateMutex(NULL, FALSE, _T("mutex_liveupdate_46dad5a1cd758470"));
		//CloseHandle(hObject);
		return true;
	}
...
void CAntiMultiOpen::minusProcessCount()
{
	...
	while (HANDLE hMutex = OpenMutex(MUTEX_ALL_ACCESS, false, _T("mutex_liveupdate_46dad5a1cd758470")))
	{
